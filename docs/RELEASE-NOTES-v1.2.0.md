...
composition close, makes every no-match rejection name exactly what didn't match, and
restores the scheme-aware authentication-boundary classification lost in the
Foundation Reset.

Requires `Cirreum.AuthenticationProvider` 1.4.0 (coordinated upgrade).
...
**Boundary classification, restored.** Before the Foundation Reset, composing
authorization registered a resolver that classified callers on the configured
`PrimaryScheme` as `Global` (operator staff) and every other authenticated scheme —
external IdPs, API keys, signed requests — as `Tenant`. The reset dropped that
registration, so every user state has stamped `AuthenticationBoundary.None` since,
and grant providers gating on `Global` or `Tenant` could never pass.
`AddAuthentication()` now registers `PrimarySchemeAuthenticationBoundaryResolver`
alongside the policies that read `PrimaryScheme`. `TryAdd` semantics throughout: an
application-registered `IAuthenticationBoundaryResolver` (now in `Cirreum.Kernel`,
namespace `Cirreum.Security`) always wins.

## Compatibility

- **Coordinated upgrade:** requires `Cirreum.AuthenticationProvider` 1.4.0 (this
  release compiles against the registration record and no longer registers the
  removed audience map service; 1.4.0 also relocates boundary resolution to
  `Cirreum.Kernel`).
- **Boundary values change on upgrade.** Hosts move from the all-`None` regression
  to real Global/Tenant classification — code that accidentally depended on `None`
  for authenticated callers sees the corrected values.
